        ...
        <div class="container2 max-w-cont">
            ...
            >
                <div class="h-[3rem] px-2 w-[9rem] max-lg:h-[2.5rem] tab:w-[7rem]">
                    <select x-model="sort" class="rounded-lg border-cc w-full h-full outline-none cursor-pointer" name="search_section" id="search_section" title="검색 분류">
                        <option value="">제목</option>
                        <option value="p_sec_1">제목+내용</option>
                    </select>
                </div>
                <div class="relative h-[3rem] ml-3 w-[20rem] max-lg:h-[2.5rem] tab:ml-1">
                    <input x-model="search" @keyup.enter="$wire.articleSearch" class="rounded-lg border-cc w-full h-full pr-[2rem] p-2 outline-none" placeholder="검색어를 입력하세요" type="search" value="">
                    ...
                    <ul class="border-t-[3px] text-lg	font-bold font-titleO tab:font-noto">
                        @foreach($generals as $general)
                            <li class="border-b border-cc">
                                <a href="{{route('boards.general-article', ['id' => $general->id])}}" class="py-[1.6rem] text-center flex flex-wrap max-lg:py-[1.3rem] tab:text-left tab:px-[0.5rem]">
                                <span class="block text-xl w-[3rem] text-grizzle-800 max-lg:text-inhe tab:w-fit tab:mr-[0.5rem] tab:text-sm">{{ $general->id }}</span>
                                <span class=" block text-xl text-point-100 w-[10rem] border-x border-grizzle-300 max-lg:text-inhe max-lg:w-[9rem] tab:w-fit tab:border-x-0 tab:text-sm">{{ \Carbon\Carbon::parse($general->created_at)->format('Y.m.d') }}</span>
                                <span class="relative block pl-[2rem] pr-[3rem] w-calcN text-left max-lg:w-calcN-lg tab:w-full tab:pl-0 tab:pr-[2.3rem] tab:mt-[0.5rem]">
                                    <span class="block font-nanumS txtOver1 tab:font-noto tab:font-normal">{{ $general->title }}</span>
    ...
    >
        <x-jet-dialog-modal wire:model="writeArticleModal">
            ...
            <x-slot name="content">
                ...
                <textarea wire:model.defer="content" type="text" rows="10" class="mt-4 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block w-full resize-none" placeholder="글을 작성해 주세요."></textarea>
            </x-slot>
            <x-slot name="footer">
